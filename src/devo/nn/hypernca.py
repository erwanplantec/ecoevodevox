...
class PerceptionModel(eqx.Module):
    ...
    def __init__(self, channels: int, perception_channels, mix_channels: bool=False, 
        ...
        self.convnet = nn.Conv2d(in_channels=channels, 
                                 out_channels=perception_channels, 
                                 kernel_size=3, 
                                 stride=1, 
                                 padding="SAME", 
...
class NCA(eqx.Module):
    ...
    def __init__(self, channels: int, perception_channels: int, update_layers: int=2, 
        ...
        self.update_model = UpdateModel(channels, perception_channels, update_layers, 
...
class NeuronCTRNNNCA(IndirectCTRNN):
    ...
    def __init__(self, size: int, synapse_channels: int, extra_channels: int, perception_channels: int,
        ...
        channels = synapse_channels + extra_channels + 2; assert channels > 4
        key_nca, key_rules = jr.split(key)
        self.nca = NCA(channels, perception_channels, update_layers, update_kws, perception_kws, key=key_nca)
    ...
    def init(self, key: jax.Array) -> IndirectCTRNNState:
        ...
        X_init = jnp.concatenate([
            ...
            jnp.abs(x_axis * 2. - 1.),
            jnp.abs(y_axis * 2. - 1.),
            jnp.zeros((self.channels-4, self.size, self.size))
        ], axis=0)
