...
class RANDCTRNNState(IndirectCTRNNState):
    x: jax.Array
    s: jax.Array
    m: jax.Array

class RAND_CTRNN(IndirectCTRNN):
    # --- GRN prms
    W_in: jax.Array
    W_ex: jax.Array
    W_migr: jax.Array
    ...
    def __init__(self, init_neurons=1, max_neurons=128, regulatory_genes=8, migratory_genes=4, signalling_genes=2, sensory_genes=1, motor_genes=1,
        ...
        Args:
            init_neurons (int): Initial number of neurons. Defaults to 1.
        ...
        """
        super().__init__(activation_fn=ctrnn_activation, dt=ctrnn_dt, T=ctrnn_T)
    ...
    def init_embryo(self, key: jax.Array)->RANDDevelopmentalState:
        ...
        Returns:
            RANDDevelopmentalState: Initial state containing gene expression levels (S), positions (X),
                      neuron mask, mitotic factors, and death factors.
        """
        S = jnp.zeros((self.max_neurons, self.total_genes))
        if self.init_neurons==1:
            X = jnp.zeros((self.max_neurons, 2))
        else:
            X = jr.normal(key, (self.max_neurons, 2))*0.01
        mask = jnp.arange(self.max_neurons) < self.init_neurons
        mitotic_factors = jnp.zeros((self.max_neurons,))
        apoptosis_factors = jnp.zeros((self.max_neurons,))
        return RANDDevelopmentalState(S, X, mask, mitotic_factors, apoptosis_factors)
    # ---
    def step(self, state: RANDDevelopmentalState, key: jax.Array) -> tuple[RANDDevelopmentalState, dict]:
        ...
        Args:
            state (RANDDevelopmentalState): Current state of the system.
            key (jax.Array): JAX random key for stochastic operations.
            
        Returns:
            tuple: (RANDDevelopmentalState, dict) containing:
                - Updated state
        ...
        def _mitosis(state: RANDDevelopmentalState, mitotic, key):
            ...
            buffer_ids = jnp.where(mitosis_mask, buffer_ids, self.max_neurons+1)
            mitotic_ids = jnp.where(mitosis_mask, mitotic_ids, self.max_neurons+1)
    ...
    # ---
    def do_migration(self, state: RANDDevelopmentalState, key: jax.Array, with_trace: bool=False)->tuple[RANDDevelopmentalState,PyTree]:
        def _step(state, key):
            state, data = self.step(state, key)
            aux = data if with_trace else None
            return state, aux
        state, trace = jax.lax.scan(lambda s, k: self.step(s, k), state, jr.split(key, self.dev_iters))
        return state, trace
    # ---
    def make_network(self, state: RANDDevelopmentalState)->IndirectCTRNNState:
        ...
        Args:
            state (RANDDevelopmentalState): Current developmental state.
    ...
    def init(self, key: jax.Array)->IndirectCTRNNState:
        ...
        state = self.init_embryo(key_init)
        state, _ = self.do_migration(state, key_migr)
        return self.make_network(state)
    
if __name__ == "__main__":
    import matplotlib.pyplot as plt
    mdl = RAND_CTRNN(init_neurons=1, key=jr.key(2))
    net = mdl.init(jr.key(1))
